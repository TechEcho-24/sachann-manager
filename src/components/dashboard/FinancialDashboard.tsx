...
export function FinancialDashboard({ role, userName }: FinancialDashboardProps) {
  const now = new Date();
  ...
  // Super Admin Tab State (Sales/Expenses by default)
  const isAdminManager = role === "admin_manager";
  const [activeTab, setActiveTab] = useState<"sales" | "tasks">("sales");

  const profitMessages = useMemo(
    () => [
      {
        title: "Awesome! You're in Profit! 🎉",
        desc: "Your sales are exceeding your expenses by <bold>. Keep up the amazing work!",
      },
      {
        title: "Great going! 🚀",
        desc: "You've made a net profit of <bold>. Consistency is the key to success!",
      },
      {
        title: "Look at you go! 🌟",
        desc: "You are ahead by <bold>. Your hard work is paying off brilliantly!",
      },
      {
        title: "Profit Mode: ON 💰",
        desc: "You've successfully secured <bold> in profit. Keep scaling up!",
      },
    ],
    []
  );
  ...
  const lossMessages = useMemo(
    () => [
      {
        title: "You're behind by <bold>",
        desc: "Every big business takes time to build. Focus on growing those sales, you can do it! 💪",
      },
      {
        title: "Current Gap: <bold>",
        desc: "Don't stress, investments are necessary for growth. Keep pushing forward! 🌱",
      },
      {
        title: "You're down by <bold>",
        desc: "Every setback is a setup for a comeback. You've got this! ✨",
      },
      {
        title: "Investment Phase 📈",
        desc: "You are currently behind by <bold>. Rome wasn't built in a day, keep building!",
      },
    ],
    []
  );
  ...
  return (
    <div className="space-y-6">
      {/* Top Tab Switcher for Super Admin */}
      {isAdminManager && (
        <div className="flex items-center gap-1.5 p-1 bg-muted/70 dark:bg-muted/40 backdrop-blur-md rounded-2xl border border-border/80 w-fit">
          <button
            type="button"
      ...
      {isAdminManager && activeTab === "tasks" ? (
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">
                Overview of your financial performance & sales
              </p>
            </div>
            <MonthSelector
              month={month}
          ...
          {/* Lifetime Stats */}
          {!loading && lifetime && (
            <div className="bg-gradient-to-r from-slate-900 to-slate-800 dark:from-slate-800 dark:to-slate-900 rounded-2xl p-5 sm:p-6 text-white shadow-lg">
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {lifetime.payerBreakdown.slice(0, 4).map((p) => (
                        <div
                          key={p.payer}
                          className="bg-white/10 backdrop-blur-sm rounded-lg px-3 py-1.5 text-xs sm:text-sm"
                        >
